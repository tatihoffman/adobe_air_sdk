...
KEYSTORE_FILE=`cd ${EXAMPLE_CI_DIR}; find . -name "*.pfx" -print` # Get any keystore file with extension .pfx
...
rm -rfv ${EXAMPLE_CI_DIR}/lib/Adjust-*.*.*.ane
...
sdk_plugin/build.sh
testing_plugin/build.sh
mkdir -p ${EXAMPLE_CI_DIR}/lib
cp -v Adjust-${VERSION}.ane ${EXAMPLE_CI_DIR}/lib/
cp -v Adjust-testing-${VERSION}.ane ${EXAMPLE_CI_DIR}/lib/

echo -e "${GREEN}>>> Checking if ANE is built successfully in location: ${EXAMPLE_CI_DIR}/lib/Adjust-${VERSION}.ane ${NC}"

if [ ! -f "${EXAMPLE_CI_DIR}/lib/Adjust-${VERSION}.ane" ]; then
